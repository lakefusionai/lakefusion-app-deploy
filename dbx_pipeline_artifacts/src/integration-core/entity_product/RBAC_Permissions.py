...
# Parameter extraction — mirrors entity_reference/RBAC_Permissions
catalog_name = dbutils.widgets.get("catalog_name")
entity = dbutils.jobs.taskValues.get("Parse_PIM_Entity_JSON", "entity")
experiment_id = dbutils.widgets.get("experiment_id")
entity_type = dbutils.jobs.taskValues.get("Parse_PIM_Entity_JSON", "entity_type", default="product", debugValue="product")
storage_type = dbutils.jobs.taskValues.get("Parse_PIM_Entity_JSON", "storage_type", default="delta", debugValue="delta")
rbac_owner_emails_raw = dbutils.jobs.taskValues.get("Parse_PIM_Entity_JSON", "rbac_owner_emails", default="[]", debugValue="[]")
rbac_owner_emails = json.loads(rbac_owner_emails_raw) if isinstance(rbac_owner_emails_raw, str) else rbac_owner_emails_raw
pim_table_schemas_raw = dbutils.jobs.taskValues.get(
    "Parse_PIM_Entity_JSON", "pim_table_schemas",
    default=dbutils.widgets.get("pim_table_schemas") or "{}",
    debugValue=dbutils.widgets.get("pim_table_schemas") or "{}",
)
pim_table_schemas = json.loads(pim_table_schemas_raw) if isinstance(pim_table_schemas_raw, str) else pim_table_schemas_raw
...
# logger is provided by execute_utils (loaded via %run above), so the banner must
# come after that cell — not before, or `logger` is undefined.
logger.info("=" * 80)
logger.info("RBAC PERMISSIONS — PIM PRODUCT ENTITY")
logger.info("=" * 80)
logger.info(f"  Entity: {entity}")
logger.info(f"  Entity type: {entity_type}")
logger.info(f"  Storage type: {storage_type}")
logger.info(f"  Catalog: {catalog_name}")
logger.info(f"  Lakebase instance: {lakebase_instance_id}")
logger.info(f"  PIM tables: {len(pim_table_schemas)}")
logger.info("=" * 80)
...
if rbac_owner_emails:
    logger.info(f"  Adding creator emails to groups: {rbac_owner_emails}")
    for grp_name in groups.values():
        add_group_members_by_email(api, w, grp_name, rbac_owner_emails)

# COMMAND ----------

# DBTITLE 1,Step 2 — Apply UC grants on Delta PIM tables
rows = load_grants(
    entity=entity,
...
for resource_type in ("catalog", "schema", "table", "volume"):
    ...
    logger.info(f"  {resource_type}: {uc_counts[resource_type]['granted']} granted, {uc_counts[resource_type]['skipped']} skipped")

# COMMAND ----------

# DBTITLE 1,Step 3 — Lakebase project + Postgres grants (Lakebase storage only)
if storage_type == "lakebase":
    logger.info("  Lakebase storage — applying project + Postgres grants")
    ...
    if lakebase_instance_id:
        try:
            ...
            logger.warning(f"  Failed granting Lakebase project access: {e}")
    else:
        logger.info("  Skipping Lakebase project grant — lakebase_instance_id not configured")

    # Postgres table grants — all PIM synced tables (derived from the manifest,
    # not hardcoded). Every pim_* table is Delta+synced (Create_PIM_Tables), so
    # each has a {entity}_{table}_prod_synced Postgres table to grant on.
    entity_snake = entity.lower().replace(" ", "_")
    pg_schema = "gold"
    synced_tables = [
        f"{entity_snake}_{table_name}_prod_synced"
        for table_name in sorted(pim_table_schemas.keys())
    ]
    try:
        pg = LakebasePgClient(
        ...
        logger.info(f"  Postgres grants applied for {len(synced_tables)} tables")
    except Exception as e:
        logger.warning(f"  Failed applying Postgres grants: {e}")
else:
    logger.info("  Delta storage — UC grants applied in Step 2, no Postgres grants needed")

# COMMAND ----------

# DBTITLE 1,Done
logger.info("=" * 80)
logger.info("RBAC PERMISSIONS — PIM PRODUCT ENTITY COMPLETE")
logger.info("=" * 80)
logger.info(f"  Entity: {entity}")
logger.info(f"  Storage type: {storage_type}")
...
dbutils.notebook.exit(json.dumps({
    "status": "success",
    "message": f"RBAC permissions granted for PIM product entity '{entity}'",
    "metrics": {"entity": entity, "entity_type": entity_type, "storage_type": storage_type,
                "synced_tables": len(pim_table_schemas)},
}))
