...
# Golden Dedup uses unified_deduplicate
...
# Filter: records with search_results populated but scoring_results empty
# No record_status in unified_dedup - only filter by scoring_results
...
if deteministic_unified_table_exists:
    ...
    u.search_results
  from
    {unified_dedup_table} u left anti join {unified_deteministic_table} ud on u.{master_id_key} = ud.{master_id_key}
      where
...
#expected_results = records_to_process * max_potential_matches

logger.info(f"Filter: Records with search_results and empty scoring_results")
# print(f"  Records to process: {records_to_process}")
# print(f"  Expected results: {expected_results} ({max_potential_matches} matches per record)")

if records_to_process:
    logger.info("No new records to process")

    # Update processed_unified_dedup from scoring_results even if no new LLM processing needed
    logger.info("\n" + "="*80)
    logger.info("STEP 5: UPDATE PROCESSED UNIFIED DEDUP FROM SCORING RESULTS")
    logger.info("="*80)
    
    # Count records with scoring_results (no record_status filter)
    ...
    if with_scoring > 0:
        # Rebuild processed_unified_dedup from scoring_results
        # FIXED: Use distinct column aliases to avoid COLUMN_ALREADY_EXISTS error
        rebuild_query = f"""
        select
        ...
        # Overwrite processed_unified_dedup table
        df_from_scoring.write.mode("overwrite").option("mergeSchema", "true").saveAsTable(processed_unified_dedup_table)
        #print(f"✓ Processed unified dedup table updated with {result_count} records")
    else:
...
# FIXED: Use literal field name 'lakefusion_id' from struct definition
# The struct has a field literally named 'lakefusion_id', not a variable
llm_query = f"""
with m as (
  select
    {master_id_key},
...
from
  ...
  llm_results
where
  uf.{master_id_key} = llm_results.{master_id_key}
  {union_clause}
...
# Group by query_lakefusion_id (the record being scored)
...
# Rebuild processed_unified_dedup from ALL records with scoring_results
# No record_status filter - unified_dedup doesn't have this column
# FIXED: Use distinct column aliases query_lakefusion_id and match_lakefusion_id
rebuild_query = f"""
select
  u.{master_id_key} as query_{master_id_key},
  exploded.lakefusion_id as match_{master_id_key},
...
# Overwrite processed_unified_dedup table (full rebuild)
