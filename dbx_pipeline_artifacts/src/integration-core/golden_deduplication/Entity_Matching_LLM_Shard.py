...
# ======================================================================
# Entity_Matching_LLM_Shard  (golden_deduplication)  — REFACTOR 1 shard worker
# ======================================================================
...
# the splitter's input table ({unified_dedup_table}_llm_input WHERE shard_index = i),
# runs the chunked ai_query (with a per-chunk retry for transient failures), and
# writes the raw scored rows to {unified_dedup_table}_llm_temp_v1_shard_i (+ a _done
# marker), then exits.
#
...
# unified error table.
#
# GOLDEN DEDUP is SINGLE-SOURCE: the source id key is lakefusion_id, with NO
# record_status filter. The Split already applied the deterministic / eligibility
# filter, so this notebook does NOT re-apply it — it scores its input partition as-is.
#
# SCORING IS INLINE (NOT %run llm_scoring_common): the prompt construction,
# response_format_clause, model-params validation, and build_ai_query_select() below
# are copied VERBATIM from Entity_Matching_LLM.py so the sharded path scores BYTE-
# IDENTICALLY to the shard_count==1 worker. KEEP these in sync with that worker.
# ======================================================================
...
# DBTITLE 1,Set Keys + Table Names (Golden Dedup = Single Source)
# GOLDEN DEDUP is single-source: source id key and master id key are both
# lakefusion_id; merged-description column is attributes_combined.
...
# Table naming matches Entity_Matching_LLM.py / Entity_Matching_LLM_Split.py exactly.
...
temp_llm_table = f"{unified_dedup_table}_llm_temp_v1_shard_{shard_index}"
...
# DBTITLE 1,Read this shard's input partition (Split already applied the eligibility filter)
...
# DBTITLE 1,STEP 2.5: Warm Up LLM Endpoint (Scale-from-zero handling)
logger.info("\n" + "=" * 80)
...
# DBTITLE 1,STEP 5: Build LLM Query (prompt + ai_query — verbatim from Entity_Matching_LLM.py)
logger.info("\n" + "=" * 80)
...
# DBTITLE 1,Build per-chunk scoring query over this shard's input partition
# Source FROM the _llm_input partition view (NOT the full table) and WITHOUT
# re-applying the deterministic / eligibility filter — the Split already did that.
# The LEFT ANTI JOIN on temp_llm_table provides crash-resume: rows already scored in a
# prior (crashed) run are skipped, so re-running resumes where it left off.
def build_partition_scoring_query(chunk_id=None, chunk_count=None):
    chunk_filter = ""
    if chunk_id is not None and chunk_count is not None:
        chunk_filter = f"WHERE abs(hash(u.{source_id_key})) % {chunk_count} = {chunk_id}"
...
# purely via the _done marker table above — Entity_Matching_LLM_Assemble's completeness
# gate checks for {shard_table}_done, so no task values are needed here.
logger.info(
