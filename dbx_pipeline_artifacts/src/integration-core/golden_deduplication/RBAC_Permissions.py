...
lakebase_ref_entities = {}  # {ref_entity_name: [synced_table_names]}
ref_entity_names = set()    # unique reference entity names for SCIM group resolution
for cfg in (rdm_configs_list or []):
    ...
    # Track Lakebase reference entities for Postgres grants
...
if ref_tables_to_grant:
    ...
    # Grant to master entity's groups AND each reference entity's groups
...
# Lakebase Postgres grants for reference entities on Lakebase storage
