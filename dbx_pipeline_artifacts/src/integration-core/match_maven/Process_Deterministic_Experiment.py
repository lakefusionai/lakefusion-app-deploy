...
# ADD CAPIQ ID
#attributes.append('capiq_id') # CapIQ ID not a match workaround
...
def build_dynamic_struct(x_col, attributes, entity_attributes_datatype, complex_attrs=None):
    ...
    for idx, attr in enumerate(attributes):
        ...
        # Only cast to a real Spark SCALAR type. Skip:
        #  - complex declared types ("STRUCT"/"ARRAY"/"MAP" and their <...> forms)
        #    — carried as JSON strings, raise INCOMPLETE_TYPE_DEFINITION if cast.
        #  - LakeFusion attribute types that aren't Spark types (e.g.
        #    "REFERENCE_ENTITY") — Spark raises UNSUPPORTED_DATATYPE. Their value
        #    is a reference id, so keep it as the trimmed string.
...
SOURCE_ID = source_id_key   # ← change if your source id column is named differently
helper_cols = {
    SOURCE_ID, "search_results", "search_results_array",
...
df_exploded = df_exploded.withColumn(
    ...
    build_attr_combined_noprefix(source_cols, SOURCE_ID),
