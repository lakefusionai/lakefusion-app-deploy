...
if is_single_source:
    ...
    source_id_key = "query_lakefusion_id"
...
potential_match_table = f"{catalog_name}.silver.{entity}_potential_match"
...
# DBTITLE 1,STEP 2: Verify Processed Table Exists
logger.info("\n" + "=" * 80)
logger.info("STEP 2: VERIFY PROCESSED TABLE")
logger.info("=" * 80)

try:
    df_processed = spark.table(processed_table)
    #processed_count = df_processed.count()
    processed_count = df_processed.isEmpty()
    logger.info(f"Processed table exists: {processed_table}")
    logger.info(f"  Check if there are records: {processed_count}")

    if processed_count == 0:
        logger.warning("No records in processed table - nothing to process")
        dbutils.notebook.exit(json.dumps({
            "status": "skipped",
            "message": "No records in processed table",
            "potential_matches": 0
        }))
except Exception as e:
    error_msg = f"Processed table not found: {str(e)}"
...
# EXPERIMENT MODE: No match status filtering - show ALL matches
if is_single_source:
    ...
    FROM {processed_table} pu
    JOIN {master_table} m ON pu.match_lakefusion_id = m.lakefusion_id
    ...
    # Use source_path instead of dataset.name (dataset struct doesn't exist in experiment unified table)
    potential_match_query = f"""
    SELECT
        m.lakefusion_id,
        m.attributes_combined,
        ...
        COUNT(*) as match_count
    FROM {processed_table} pu
    JOIN {source_table} u ON pu.surrogate_key = u.surrogate_key
    JOIN {master_table} m ON pu.lakefusion_id = m.lakefusion_id
    WHERE pu.lakefusion_id IS NOT NULL
    AND u.record_status = 'ACTIVE'
    GROUP BY m.lakefusion_id, m.attributes_combined, {master_attrs}
    """

logger.info(f"Query built for {mode_name}")
...
try:
    ...
    #logger.info(f"Found {potential_count} master records with potential matches")
...
# DBTITLE 1,STEP 6: Summary Statistics
logger.info("\n" + "=" * 80)
logger.info("STEP 6: SUMMARY STATISTICS")
logger.info("=" * 80)

# Get statistics
# total_potential = spark.table(potential_match_table).count()
# total_matches = spark.sql(f"""
#     SELECT SUM(match_count) as total FROM {potential_match_table}
# """).collect()[0]['total'] or 0

# print(f"  Master records with matches: {total_potential}")
# print(f"  Total potential matches: {int(total_matches)}")
# print(f"  Average matches per master: {total_matches / total_potential if total_potential > 0 else 0:.2f}")

# COMMAND ----------

# DBTITLE 1,Set Task Values and Exit
# dbutils.jobs.taskValues.set("potential_match_complete", True)
# dbutils.jobs.taskValues.set("potential_match_count", int(total_matches))
...
logger.info(f"Potential Match Table: {potential_match_table}")
# logger.info(f"Master records: {total_potential}")
# logger.info(f"Total matches: {int(total_matches)}")
logger.info("")
logger.warning("EXPERIMENT MODE:")
logger.warning("   - ALL matches shown (no match status filtering)")
logger.warning("   - Use scores to analyze match quality")
logger.info("=" * 80)
...
dbutils.notebook.exit(json.dumps({
    "status": "success",
    "mode": mode_name
    #"potential_match_table": potential_match_table,
    #"master_records": total_potential,
