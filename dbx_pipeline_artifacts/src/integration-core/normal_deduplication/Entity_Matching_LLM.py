...
#      crash-safe materialization. Each chunk commits independently.
...
#      Unparseable or incomplete results are retried, then nulled.
...
# Advanced LLM config — task values from Parse Entity JSON take priority over widget defaults
...
temp_llm_table = f"{unified_table}_llm_temp_v1"
...
if records_to_process == 0:
    ...
    if active_with_scoring_count > 0:
        rebuild_query = f"""
        select
          u.{unified_id_key},
        ...
        if not spark.catalog.tableExists(processed_unified_table):
            logger.info(f"  Creating empty processed_unified table: {processed_unified_table}")
            empty_schema = StructType([
                StructField(unified_id_key, StringType(), True),
            ...
            empty_df.write.mode("overwrite").option("mergeSchema", "true").saveAsTable(processed_unified_table)
            logger.info("  ✓ Empty processed_unified table created")
...
# Build LLM query as a function that supports chunk filtering and anti-join for resume
# (temp_llm_table is defined earlier alongside the other table names so STEP 4 marker
# check and STEP 12 marker write share it)
...
def build_llm_query(chunk_id=None, chunk_count=None):
    ...
    # Use LEFT ANTI JOIN instead of NOT IN subquery for better performance
    temp_anti_join = ""
...
# Run the actual LLM prompt on up to 5 sample records to verify output format
...
# Idempotent resume: check how many records are already processed
already_processed = 0
if spark.catalog.tableExists(temp_llm_table):
...
for chunk_id in range(chunk_count):
    ...
    try:
        ...
        # build_llm_query's slicing predicate at line ~737. The deterministic
        ...
        try:
            failed_keys_df = spark.sql(f"""
                SELECT {unified_id_key} AS surrogate_key
                FROM {unified_table}
                WHERE record_status = 'ACTIVE'
                  AND search_results IS NOT NULL AND search_results != ''
                  AND (scoring_results IS NULL OR scoring_results = '')
                  AND abs(hash({unified_id_key})) % {chunk_count} = {chunk_id}
            """)
...
if chunk_failures:
    chunk_failure_handler = UnifiedErrorHandler(spark, unified_table)
...
# Count entries in search_results (JSON array-of-arrays from VS layer)
...
# Build retry query for specific bad records
def build_retry_query_for_keys(keys_table_name):
    ...
    WITH uf AS (
      ...
      INNER JOIN {keys_table_name} k ON u.{unified_id_key} = k.{unified_id_key}
    )
    SELECT
      {unified_id_key},
      {merged_desc_column},
...
for retry_attempt in range(1, MAX_LLM_RETRIES + 1):
    ...
    spark.table(temp_llm_table).filter(col("is_bad")) \
        .select(unified_id_key).write.format("delta").mode("overwrite").saveAsTable(retry_keys_table)
...
if final_bad_count > 0:
    ...
    # Log bad records to unified error table before nulling them
    ...
    # Null out bad results so they don't pollute downstream
