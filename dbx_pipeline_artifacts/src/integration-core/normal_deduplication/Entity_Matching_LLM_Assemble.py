...
# ======================================================================
# Entity_Matching_LLM_Assemble  (normal_deduplication)  — REFACTOR 1 fan-in
# ======================================================================
...
# then runs STEP 7-12 — explode → classify → single MERGE into the unified table
# → rebuild processed → optimize → cleanup. This is the ONLY task that writes the
# unified table, so there are no concurrent writes and no partial merges.
#
...
temp_llm_table_base = f"{unified_table}_llm_temp_v1"
...
# unionByName(allowMissingColumns=True) tolerates per-shard column-order/extra-column
# differences. STEP 7 below reads `temp_llm_table`, so point it at the assembled view.
...
# matches folded into the explode so they land in processed_unified).
union_clause = ""
...
spark.sql(f"DROP TABLE IF EXISTS {unified_table}_llm_input")
