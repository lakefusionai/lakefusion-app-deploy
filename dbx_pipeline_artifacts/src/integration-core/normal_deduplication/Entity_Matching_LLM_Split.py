...
# ======================================================================
# Entity_Matching_LLM_Split  (normal_deduplication)  — REFACTOR 1 splitter
# ======================================================================
...
# unified table, builds the deterministic _det_filter, applies the SAME eligibility
# filter the worker (Entity_Matching_LLM.py STEP 4) applied per-shard, and writes the
# eligible rows ONCE into a single input table partitioned by shard_index:
#
#     {unified_table}_llm_input  PARTITIONED BY (shard_index)
#       columns: surrogate_key, attributes_combined, search_results, shard_index
#       shard_index = abs(hash(surrogate_key)) % shard_count
#
# Each Entity_Matching_LLM_Shard then reads ONLY its own partition
# (WHERE shard_index = i), so the full unified scan + deterministic join happen once
# here instead of once per shard.
#
# This notebook HARDCODES the normal-dedup (multi-source) path: is_single_source=False,
# surrogate_key id, record_status='ACTIVE' eligibility. The _det_filter build + the
# filter_query are kept BYTE-FOR-BYTE in sync with Entity_Matching_LLM.py STEP 4
# (the deterministic-table branch and the no-deterministic fallback). KEEP IN SYNC.
# ======================================================================
...
# DBTITLE 1,Set Keys (normal dedup — multi-source, hardcoded)
# Hardcoded normal-dedup mode: is_single_source = False (multi-source).
...
# Mirrors Entity_Matching_LLM.py / Entity_Matching_LLM_Assemble.py table naming.
...
# Each shard's input partition source; the Assemble unions
# {unified_table}_llm_temp_v1_shard_{i}.
...
logger.info(f"Deterministic Table: {unified_deteministic_table} (exists={deteministic_unified_table_exists})")
...
# Verbatim from Entity_Matching_LLM.py: aggregate the per-(source × candidate)
# deterministic table back into per-source shape so the LLM filter can join on
# surrogate_key:
#   - has_positive_match: any MATCH/POTENTIAL_MATCH-action row → skip LLM
#   - filtered_search_results: original search_results minus NO_MATCH'd candidate ids;
#     null when all candidates were filtered (→ JOB_INSERT, skip LLM).
...
if deteministic_unified_table_exists:
    ...
    df_source_for_filter = spark.table(unified_table).select(unified_id_key, "search_results")

    df_det_filter = (
        df_source_for_filter
        .join(df_det_agg, on=unified_id_key, how="inner")
        ...
        .select(unified_id_key, "has_positive_match", "filtered_search_results")
    )
...
# Byte-for-byte the normal-dedup eligibility predicate from Entity_Matching_LLM.py
# STEP 4: record_status='ACTIVE', non-empty search_results, scoring_results unset,
# and the deterministic join (skip positive matches; COALESCE filtered candidates).
if deteministic_unified_table_exists:
    filter_query = f"""
      select
        u.{unified_id_key},
...
(
    ...
    .withColumn("shard_index", F.expr(f"abs(hash({unified_id_key})) % {effective_shard_count}"))
    .write.format("delta")
    .mode("overwrite")
