...
if len(rules_config)==0:
  logger.info("No deterministic rules found — exiting")
  logger_instance.shutdown()   # flush logs to volume before dbutils.notebook.exit
...
# Final flush of the in-memory log buffer to the pipeline_logs volume.
# Without this, logs from the tail of the run can be lost on notebook exit.
