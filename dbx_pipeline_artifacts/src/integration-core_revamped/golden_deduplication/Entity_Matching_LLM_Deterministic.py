...
# MAGIC %md
# MAGIC # Golden Dedup - Entity Matching LLM Deterministic (SDK Version)
# MAGIC
# MAGIC Uses LLM with deterministic rules for master-to-master matching.
# MAGIC
...
logger.info("="*60)
logger.info("GOLDEN DEDUP - LLM DETERMINISTIC MATCHING (SDK VERSION)")
logger.info("="*60)
...
from lakefusion_core_engine.executors.tasks.integration_core.golden_deduplication import GoldenLLMMatchingDeterministicExecutor

# Create task context
context = TaskContext(
    task_name="golden_llm_matching_deterministic",
    entity=entity,
...
executor = GoldenLLMMatchingDeterministicExecutor(context)
