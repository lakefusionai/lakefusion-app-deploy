...
# MAGIC %md
# MAGIC # Golden Dedup - Entity Matching Vector Search (SDK Version)
# MAGIC
# MAGIC Performs vector similarity search for golden deduplication - matching master
# MAGIC records against other masters (excluding self-matches).
# MAGIC
...
logger.info("="*60)
logger.info("GOLDEN DEDUP - VECTOR SEARCH (SDK VERSION)")
logger.info("="*60)
...
from lakefusion_core_engine.executors.tasks.integration_core.golden_deduplication import GoldenVectorSearchExecutor

# Create task context
context = TaskContext(
    task_name="golden_vector_search",
    entity=entity,
...
executor = GoldenVectorSearchExecutor(context)
