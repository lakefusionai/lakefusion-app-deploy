...
# MAGIC %md
# MAGIC # Increment Updates to Unified (SDK Version)
# MAGIC
# MAGIC Processes UPDATE incrementals from ALL source tables with survivorship recalculation
# MAGIC for multi-contributor scenarios. Updates unified and master tables.
# MAGIC
# MAGIC **Customization Points:**
# MAGIC - `PRE-EXECUTE`: Add custom logic before processing updates
# MAGIC - `POST-EXECUTE`: Add custom logic after processing updates
...
has_updates = dbutils.jobs.taskValues.get(taskKey="Check_Increments_Exists", key="has_updates", debugValue=True)
...
logger.info("=" * 60)
logger.info("INCREMENT UPDATES TO UNIFIED (SDK VERSION)")
logger.info("=" * 60)
...
from lakefusion_core_engine.executors.tasks.integration_core.incremental_load import IncrementUpdatesExecutor

# Create task context
context = TaskContext(
    task_name="increment_updates_to_unified",
    entity=entity,
...
executor = IncrementUpdatesExecutor(context)
