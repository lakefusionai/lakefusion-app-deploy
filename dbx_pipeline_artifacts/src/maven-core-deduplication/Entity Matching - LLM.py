...
if(is_golden_deduplication==1 or experiment_id=='prod'):
    incremental_load = dbutils.jobs.taskValues.get(taskKey="Prepare_Tables_DeDuplicate", key="incremental_load", debugValue=incremental_load)
else:
...
if incremental_load and (experiment_id != 'prod'):
    ...
    llm_endpoint = dbutils.jobs.taskValues.get(
        taskKey="Endpoints_Mapping_DeDuplicate", key="llm_model_endpoint", debugValue=llm_endpoint
    )
    embedding_endpoint = dbutils.jobs.taskValues.get(
        taskKey="Endpoints_Mapping_DeDuplicate", key="embedding_model_endpoint", debugValue=embedding_endpoint
    )
...
if base_prompt is None or base_prompt == "":
    ...
    from
      er inner join m
      on er.exploded_result.`lakefusion_id` = m.lakefusion_id and  m.{id_key}<>er.{id_key};
    """
else:
    if processed_unified_table_exists:
      uf_sub_query = f"""uf as (
...
processed_unified_table_df=spark.read.table(processed_unified_table)
num_cols = len(processed_unified_table_df.columns)+1
spark.sql(f"ALTER TABLE {processed_unified_table} SET TBLPROPERTIES ('delta.dataSkippingNumIndexedCols' = '{num_cols}')")
optimise_res = spark.sql(f"OPTIMIZE {processed_unified_table} ZORDER BY ({id_key}, {master_id_key_column_name})")
