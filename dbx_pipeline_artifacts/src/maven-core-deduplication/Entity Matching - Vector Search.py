...
try:
    ...
    existing_endpoints = [ep.get("name") for ep in endpoints if "name" in ep]
    ...
    if vs_endpoint in existing_endpoints:
        ...
        try:
            ...
            logger.error(f"Exception occurred while creating the VSC endpoint: {e}")

except Exception as outer_e:
    logger.error(f"Failed to fetch endpoint list: {outer_e}")
    ...
    if "RESOURCE_ALREADY_EXISTS" in error_msg or "already exists" in error_msg.lower():
        ...
        try:
            ...
            if "scale to zero" in sync_error_msg.lower():
                logger.error(
                    "Error: The embedding model endpoint has 'Scale to zero' enabled. "
