...
filter_expr = reduce(
    lambda acc, item: acc | ((col(f'unified_{id_key}') == item['id']) & (col('unified_dataset_id') == item['dataset_id'])),
    unified_dataset_ids,
    lit(False)  # Initial accumulator for the reduce function
)

master_filter = col(f'master_{id_key}') == master_id

# Apply both filters to the DataFrame
filtered_df = potential_matches_df.filter(filter_expr & master_filter)

# COMMAND ----------

# First, identify all potential matches for the unified records we're processing
unified_ids_to_process = [item['id'] for item in unified_dataset_ids]
unified_dataset_ids_to_process = [item['dataset_id'] for item in unified_dataset_ids]

# Get all potential matches for these unified records (not just the ones we're merging)
all_potential_matches_filter = reduce(
    lambda acc, item: acc | ((col(f'unified_{id_key}') == item['id']) & (col('unified_dataset_id') == item['dataset_id'])),
    unified_dataset_ids,
    lit(False)
)

all_potential_matches_df = potential_matches_df.filter(all_potential_matches_filter)

# Create the NOT_A_MATCH records: all matches except the one with the specified master_id
not_a_match_filter = col(f'master_{id_key}') != master_id

not_a_match_df = all_potential_matches_df.filter(not_a_match_filter)

master_version = get_current_table_version(master_table)

# Log the NOT_A_MATCH activities
resultant_not_a_match_activities = (
    not_a_match_df
    .select(
        col(f'master_{id_key}').alias(f'master_{id_key}'),
...
def apply_aggregation_strategy(attribute, master_record, unified_records, entity_attributes_datatype, ignore_null=False, aggregate_unique_only=False):
    ...
    # Handle master value based on datatype
    if is_string_type(attr_datatype) and master_value:
        # For strings, split by separator if it exists
        if ignore_null and (master_value is None or master_value == ''):
            pass
        else:
            master_values = master_value.split(aggregation_strategy_separator)
    else:
        # For numeric/datetime, treat as single value
        if ignore_null and (master_value is None or master_value == ''):
            pass
        else:
            master_values = [master_value] if master_value is not None else []
    ...
    if is_numeric_type(attr_datatype):
        ...
        result_value = aggregation_strategy_separator.join(all_values)
        return result_value
...
def process_survivorship_rules(unified_records, master_record, unified_datasets, master_attribute_source_mapping, entity_attributes_datatype):
    ...
    for attribute in entity_attributes:
        ...
        current_value = master_record.get(attribute)
        current_source = master_attribute_source_mapping_column.get('source') if master_attribute_source_mapping_column else None
        ...
        if rule:
            ...
            ignore_null = rule.get('ignoreNull', False)  # Get ignore null setting
            
            if strategy == 'Source System':
                ...
                # Combine all sources for aggregation
                sources = []
                if current_source:
                    sources.append(current_source)
                sources.extend([path for path in unified_datasets if path])
                ...
                resultant_record[attribute] = current_value
        ...
        value_to_store = resultant_record.get(attribute)
        resultant_master_attribute_source_mapping.append({
            'attribute_name': attribute,
            'attribute_value': str(value_to_store) if value_to_store is not None else None,
            'source': source
        })
...
resultant_merge_activities_survivorship = (
    ...
    .select(
        ...
        col(f'unified_dataset_path').alias('source'),
        lit(master_version).alias('version'),
        lit(ActionType.MANUAL_MERGE.value).alias("action_type")
    )
)
